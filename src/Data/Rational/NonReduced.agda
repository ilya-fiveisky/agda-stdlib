...
module Data.Rational.NonReduced where
...
record ℚ : Set where
  ...
  field
    numerator     : ℤ
    denominator : ℕ
    .denominator≢0 : False (ℕ._≟_ denominator 0)

-- Constructs rational numbers.
...
(n ÷ zero) {≢0 = ()}
(n ÷ suc d) =
  record { numerator = n; denominator = suc d }
...
_+_ : ℚ → ℚ → ℚ
(Q (+ zero) _ _) + q2 = q2
(Q (+ (suc n)) d _) + q2 = ((+ 1) ÷ d) + (((+ n) ÷ d) + q2)

_*_ : ℚ → ℚ → ℚ
_ * (Q (+ zero) _ _) = 0/1
(Q (+ zero) _ _) * _ = 0/1
--(Q (suc n) _ _) * q2 = 
(Q n1 (suc d1) _) * (Q n2 (suc d2) _) = (n1 ℤ.* n2) ÷ ((suc d1) ℕ.* (suc d2))
--q1 * q2 = (numerator q1 ℤ.* numerator q2) ÷ (denominator q1 ℕ.* denominator q2)
  where open ℚ
-}
-- ------------------------------------------------------------------------
-- -- Equality
